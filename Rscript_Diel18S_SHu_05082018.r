...
# Run RAIN for DNA
dna_RAIN<-rain(t(as.matrix(dna_dfw)), period=24, measure.sequence=ft, deltat=4, method="independent", na.rm = TRUE)
...
# OTUs significant in both RNA and DNA libraries
# c <-subset(b, both == 2)
# head(c); length(unique(c$OTU.ID))
# c_tax<-join(c,key)
# c_tax
...
head(allsig_tax[1:3,])
write.table(allsig_tax, file="TableS3_sigRhythmicity_05032018.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
tmp3 <-join(rna_tmp2, dna_tmp2)
head(tmp3)
# Write table - TABLE 1 in manuscript
write.table(tmp3, file="Table1_OTUs_sigRhythmicity_05032018.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
head(envir)
colnames(envir)[1]<-"#"
LSA_RNA_wEnv<-rbind(LSA_rna10_allTimepoints, envir)
dim(LSA_RNA_wEnv)
head(LSA_RNA_wEnv[208:213,])
#
# Write table for LSA:
# write.table(LSA_RNA_wEnv, file="LSA_RNA_inputTable_10_05032018.txt", quote=FALSE, sep="\t", row.names=FALSE)

#Also write attribute file:
head(key)
tmp<-as.data.frame(LSA_rna10_allTimepoints$`#`); colnames(tmp)<-"OTU.ID"
key_filtered<-join(tmp, key)
# write.table(key_filtered, file="attributes_RNA_diel_Filtered10_all.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
write.csv(sum_topLSA, file="Top20_OTUpairs.csv")
# write.csv(df2, file="freq_allinteractions_05072018.csv") 
# write.csv(df_tax2, file="alltax_combinations_05072018.csv") # Additional information regarding Table S4.


# last updated - 05-07-2018 SHu


# Select Rhizarian and Syndiniales OTU interactions -----------------------
load(file="LSA_Robjects.RData", verbose=T) #Option to start here
...
# write.csv(syn_sub, file="Syndinales_putativeParasitic.csv")
...
# write.csv(rhiz_sub, file="Rhizaria_putativeMutualism.csv")
...
syn_summary
write.csv(syn_summary, file="syndinales_freq_Table3.csv") # Table 3
...
write.csv(rhiza_summary, file="Rhizaria_freq_Table3.csv") # Table 3
...
names(relAbun)
dna_base<-dcast(relAbun[c(1,13,14)], OTU.ID~Num, fill=0)
rna_base<-dcast(relAbun[c(1,13,15)], OTU.ID~Num, fill=0)
# Format for CCA analysis input
row.names(dna_base)<-dna_base$OTU.ID; dna_base$OTU.ID<-NULL
row.names(rna_base)<-rna_base$OTU.ID; rna_base$OTU.ID<-NULL
# Remove all zero rows
rowsum<-apply(dna_base[1:19],1,sum)
dna_input = dna_base[ rowsum>0, ] 
rowsum<-apply(rna_base[1:19],1,sum)
rna_input = rna_base[ rowsum>0, ]

# Plot RNA CCA plot:
library(vegan)
RNA_CCA<-cca(t(rna_input))
key<-read.delim("key_CCA.txt")# Manual curation of colors for sample names.
eig<-eigenvals(RNA_CCA)
x<-signif((eig[1]*100),3)
y<-signif((eig[2]*100),3)
# Plot with legend
plot(RNA_CCA,type=c("points"),display=c("sites", "bp"),main="RNA by sample",xlab=bquote(.(x)*"%"), ylab=(bquote(.(y)*"%")))
rna_clust<-hclust(dist(t(rna_input)))
points(RNA_CCA,pch=as.numeric(key$Shape),bg=as.character(key$Color),cex=2,lwd=2, labels=as.factor(key$Time))
legend(1.5,3.3, c("6AM","10AM","2PM","6PM","10PM","2AM"), pt.bg=c("#d7b5d8","#fed98e","#d73027","#31a354","#045a8d","#810f7c"),pch=c(24,24,24,21,21,21),cex=1,pt.cex=1.6,bty="n",y.intersp=2)
# Save CCA plots : W:520, H: 555

# Repeat for DNA results
is.na(dna_input)<-sapply(dna_input, is.infinite)
dna_input[is.na(dna_input)]<-0
DNA_CCA<-cca(t(dna_input))
key<-read.delim("key_CCA.txt")
eig<-eigenvals(DNA_CCA)
x<-signif((eig[1]*100),3)
y<-signif((eig[2]*100),3)

# Plot
plot(DNA_CCA,type=c("points"),display=c("sites"),main="DNA by sample",xlab=bquote(.(x)*"%"), ylab=(bquote(.(y)*"%")))
points(DNA_CCA,pch=as.numeric(key$Shape),bg=as.character(key$Color),cex=2,lwd=2)
